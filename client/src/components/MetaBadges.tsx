...
export const TRUST_EXPLAIN =
  "可信度评分（0–100）：由方法学披露程度、出题方立场、是否开源可复现、是否披露置信区间、污染风险等结构性因素合成。它衡量的是「这把尺子本身可不可信」，与模型分数高低无关。";

export const DISC_EXPLAIN =
  "分辨力（0–100）：衡量该指标当前还能区分多少模型差异。已被刷到接近满分的指标分辨力极低——此时排名差异多为噪声。";
...
export const UTILITY_EXPLAIN =
  "效用分（0–100）：可信度与分辨力的加权合成，回答「这个指标现在值不值得看」。发布会上最常被引用的饱和指标，效用分往往最低。";
